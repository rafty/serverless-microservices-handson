...
class CancelOrderSagaStepFunctions(Construct):
    ...
    def __init__(self, scope: Construct, id: str, props: dict) -> None:
        ...
        # ------------------------------------------------------
        # Order Service Function
        self.begin_cancel_order_saga_function = props.get('order_service_function')
        self.undo_begin_cancel_order_saga_function = props.get('order_service_function')
        self.confirm_cancel_order_saga_function = props.get('order_service_function')

        # Kitchen Service Function
        self.kitchen_service_function = self.get_function_on_cross_stack(
                                                            'KitchenServiceFunctionARN')
        self.begin_cancel_ticket_saga_function = self.kitchen_service_function
        self.undo_begin_cancel_ticket_saga_function = self.kitchen_service_function
        self.confirm_cancel_ticket_saga_function = self.kitchen_service_function

        # Account Service Function
        self.reverse_authorize_card_saga_function = self.get_function_on_cross_stack(
                                                            'AccountServiceFunctionARN')
        ...
        #  このようにしてるのは一つのLambda functionで複数のtaskを実現してるため
        # Order service Tasks
        self.begin_cancel_order_task = self.task_begin_cancel_order()
        self.undo_begin_cancel_order_task = self.task_undo_begin_cancel_order()
        self.confirm_cancel_order_task = self.task_confirm_cancel_order()

        # Kitchen service Tasks
        self.begin_cancel_ticket_task = self.task_begin_cancel_ticket()
        self.undo_begin_cancel_ticket_task = self.task_undo_begin_cancel_ticket()
        self.confirm_cancel_ticket_task = self.task_confirm_cancel_ticket()

        # Account service Tasks
        self.reverse_authorize_card_task = self.task_reverse_authorize_card()
    ...
    def create_state_machine(self):
        state_machine = aws_stepfunctions.StateMachine(
            self,
            'CancelOrderSagaStateMachine',
            state_machine_name='CancelOrderSaga',
            definition=self.cancel_order_definition(),
            timeout=Duration.minutes(3),
            tracing_enabled=True)
        return state_machine
    ...
    # task definition
    # -------------------------------------------------------
    def cancel_order_definition(self):
        definition = \
            self.begin_cancel_order_task\
                .next(self.begin_cancel_ticket_task)\
                .next(self.reverse_authorize_card_task)\
                .next(self.confirm_cancel_ticket_task)\
                .next(self.confirm_cancel_order_task)

        return definition

    # 補償トランザクション definition
    def undo_begin_cancel_order_compensation_transaction_definition(self):
        # begin_cancel_ticketが失敗したときの補償トランザクション
        definition = self.undo_begin_cancel_order_task
        return definition

    # 補償トランザクション definition
    def undo_begin_cancel_ticket_compensation_transaction_definition(self):
        # reverse_authorize_cardが失敗したときの補償トランザクション
        definition = \
            self.undo_begin_cancel_ticket_task\
                .next(self.undo_begin_cancel_order_task)
        return definition

    # -------------------------------------------------------
    # tasks
    # -------------------------------------------------------
    def task_begin_cancel_order(self):
        # ------------------------ payload ----------------------------
        task_context = {
            "state_machine": "CancelOrderSaga",
            "action": "BEGIN_CANCEL_ORDER"
        }
        payload = aws_stepfunctions.TaskInput.from_object({
            "order_id.$": "$.order_id",
        ...
        task = aws_stepfunctions_tasks.LambdaInvoke(
            self,
            'TaskBeginCancelOrder',
            lambda_function=self.begin_cancel_order_saga_function,
            payload=payload,
            result_path='$.begin_cancel_order_result',
            output_path='$')

        return task

    def task_undo_begin_cancel_order(self):
        # Cancel Order Saga - 補償トランザクション
        # ------------------------ payload ----------------------------
        task_context = {
            "state_machine": "CancelOrderSaga",
            "action": "UNDO_BEGIN_CANCEL_ORDER"
        }
        payload = aws_stepfunctions.TaskInput.from_object({
            "order_id.$": "$.order_id",
            "task_context": aws_stepfunctions.TaskInput.from_object(task_context)
        ...
        task = aws_stepfunctions_tasks.LambdaInvoke(
            self,
            'TaskUndoBeginCancelOrder',
            lambda_function=self.undo_begin_cancel_order_saga_function,
            payload=payload,
            result_path='$.undo_begin_cancel_order_result',
            output_path='$')

        return task

    def task_begin_cancel_ticket(self):
        # ------------------------ payload ----------------------------
        task_context = {
            "state_machine": "CancelOrderSaga",
            "action": "BEGIN_CANCEL_TICKET"
        }
        payload = aws_stepfunctions.TaskInput.from_object({
            "order_id.$": "$.order_id",  # order_idとticket_idは同じ値
            "task_context": aws_stepfunctions.TaskInput.from_object(task_context)
        })
        # ------------------------ payload ----------------------------
        ...
        task = aws_stepfunctions_tasks.LambdaInvoke(
            self,
            'TaskBeginCancelTicket',
            lambda_function=self.begin_cancel_ticket_saga_function,
            payload=payload,
            result_path='$.begin_cancel_ticket_result',
            output_path='$')

        task.add_catch(self.undo_begin_cancel_order_compensation_transaction_definition(),
                       errors=['UnsupportedStateTransitionException',
                               'TicketNotFoundException'],
                       result_path='$.begin_cancel_ticket_result')  # 注意 $にするとinputが上書きされる

        return task

    def task_undo_begin_cancel_ticket(self):
        # ------------------------ payload ----------------------------
        task_context = {
            "state_machine": "CancelOrderSaga",
            "action": "UNDO_BEGIN_CANCEL_TICKET"
        }
        payload = aws_stepfunctions.TaskInput.from_object({
            "order_id.$": "$.order_id",  # order_idとticket_idは同じ値
            "task_context": aws_stepfunctions.TaskInput.from_object(task_context)
        ...
        task = aws_stepfunctions_tasks.LambdaInvoke(
            self,
            'TaskUndoBeginCancelTicket',
            lambda_function=self.undo_begin_cancel_ticket_saga_function,
            payload=payload,
            result_path='$.undo_begin_cancel_ticket_result',
            output_path='$')

        return task

    def task_reverse_authorize_card(self):
        # ------------------------ payload ----------------------------
        task_context = {
            "state_machine": "CancelOrderSaga",
            "action": "REVERSE_AUTHORIZE_CARD"
        }
        payload = aws_stepfunctions.TaskInput.from_object({
            "order_id.$": "$.order_id",  # order_idとticket_idは同じ値
            "consumer_id.$": "$.consumer_id",
        ...
        task = aws_stepfunctions_tasks.LambdaInvoke(
            self,
            'TaskReverseAuthorizeCard',
            lambda_function=self.reverse_authorize_card_saga_function,
            payload=payload,
            result_path='$.reverse_authorize_card_result',
            output_path='$')

        # ここから
        task.add_catch(self.undo_begin_cancel_ticket_compensation_transaction_definition(),
                       errors=['CardAuthorizationFailedException',
                               'AccountNotFoundException'],
                       result_path='$.reverse_authorize_card_result')  # 注意 $にするとinputが上書きされる

        return task

    def task_confirm_cancel_ticket(self):
        # ------------------------ payload ----------------------------
        task_context = {
            "state_machine": "CancelOrderSaga",
            "action": "CONFIRM_CANCEL_TICKET"
        }
        payload = aws_stepfunctions.TaskInput.from_object({
            "order_id.$": "$.order_id",  # order_idとticket_idは同じ値
            "task_context": aws_stepfunctions.TaskInput.from_object(task_context)
        ...
        task = aws_stepfunctions_tasks.LambdaInvoke(
            self,
            'TaskConfirmCancelTicket',
            lambda_function=self.confirm_cancel_ticket_saga_function,
            payload=payload,
            result_path='$.confirm_cancel_ticket_result',
            output_path='$')

        return task

    def task_confirm_cancel_order(self):
        # ------------------------ payload ----------------------------
        task_context = {
            "state_machine": "CancelOrderSaga",
        ...
        task = aws_stepfunctions_tasks.LambdaInvoke(
            self,
            'TaskConfirmCancelOrder',
            lambda_function=self.confirm_cancel_order_saga_function,
            payload=payload,
            result_path='$.confirm_cancel_order_result',
            output_path='$')
